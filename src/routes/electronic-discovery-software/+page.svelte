<script>
	...
	onMount(async () => {
		const mod = await import('$lib/components/hero/EDiscoveryAnimation.svelte');
	...
	jsonLd={[
		...
		},
		{
			...
			itemListElement: [
				...
				{ '@type': 'ListItem', position: 2, name: 'E-Discovery Software', item: `https://www.dodon.ai${data.seo.canonical}` }
			]
		},
		{
			'@context': 'https://schema.org',
			'@type': 'HowTo',
			name: 'How to Search Documents with Dodonai AI eDiscovery',
